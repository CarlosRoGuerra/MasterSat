    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/50">
          <p className="text-2xs font-semibold uppercase tracking-widest text-slate-500">Documento</p>
          <p className="mt-2 font-semibold text-slate-900 dark:text-white">{formatCpfCnpj(client.cpf_cnpj)}</p>
        </div>
        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/50">
          <p className="text-2xs font-semibold uppercase tracking-widest text-slate-500">Status</p>
          <div className="mt-2"><Badge variant={statusVariant(client.status)}>{statusLabel(client.status)}</Badge></div>
        </div>
        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/50">
          <p className="text-2xs font-semibold uppercase tracking-widest text-slate-500">E-mail</p>
          <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{client.email || 'Não informado'}</p>
        </div>
        <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/50">
          <p className="text-2xs font-semibold uppercase tracking-widest text-slate-500">Telefone</p>
        ...
        <div className="rounded-xl border border-brand-100 bg-brand-50/60 p-4 dark:border-brand-900/40 dark:bg-brand-950/30">
          <p className="text-2xs font-semibold uppercase tracking-widest text-brand-500 dark:text-brand-400">Dia de vencimento</p>
      ...
      <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/50">
        <p className="text-2xs font-semibold uppercase tracking-widest text-slate-500">Endereço</p>
      ...
      {(client.contacts || []).length > 0 && (
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Contatos adicionais</p>
          <div className="space-y-2">
            {(client.contacts || []).map((c, i) => (
              <div key={i} className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/50">
                <p className="font-medium text-slate-900 dark:text-white">{c.name}{c.role && <span className="ml-2 text-xs font-normal text-slate-500">({c.role})</span>}</p>
      ...
      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Veículos vinculados</p>
        {vehicles.length === 0 ? (
          <p className="text-sm text-slate-500">Nenhum veículo vinculado.</p>
        ) : (
