...
type Contract = { id: number; client_id: number; plan_id: number; vehicle_id?: number | null; tracker_id?: number | null; start_date: string; end_date?: string | null; status: string; billing_day?: number | null; payment_method?: string | null; notes?: string | null; installation_fee?: number | null; uninstall_fee?: number | null; delivery_method?: string | null; client_name?: string | null; plan_name?: string | null; vehicle_plate?: string | null; tracker_identifier?: string | null; monthly_value?: number | null; open_billings: number; next_due_date?: string | null };
...
type ContractFormState = { client_id: string; vehicle_id: string; tracker_id: string; plan_id: string; start_date: string; end_date: string; billing_day: string; payment_method: string; notes: string; installation_fee: string; uninstall_fee: string; delivery_method: string };
...
const initialContractForm: ContractFormState = { client_id: '', vehicle_id: '', tracker_id: '', plan_id: '', start_date: new Date().toISOString().slice(0, 10), end_date: '', billing_day: '', payment_method: 'boleto', notes: '', installation_fee: '', uninstall_fee: '', delivery_method: 'email' };
...
export default function FinanceiroPage() {
  ...
  function openEditContract(contract: Contract) {
    ...
    setModalError('');
    setContractForm({ client_id: String(contract.client_id), vehicle_id: contract.vehicle_id ? String(contract.vehicle_id) : '', tracker_id: contract.tracker_id ? String(contract.tracker_id) : '', plan_id: String(contract.plan_id), start_date: contract.start_date, end_date: contract.end_date || '', billing_day: contract.billing_day ? String(contract.billing_day) : '', payment_method: contract.payment_method || 'boleto', notes: contract.notes || '', installation_fee: contract.installation_fee != null ? String(contract.installation_fee) : '', uninstall_fee: contract.uninstall_fee != null ? String(contract.uninstall_fee) : '', delivery_method: contract.delivery_method || 'email' });
      ...
      <Modal open={contractModal} onClose={() => { setContractModal(false); setEditingContractId(null); setContractForm(initialContractForm); setModalError(''); }} title={editingContractId ? 'Editar contrato' : 'Novo contrato'} description="Vincule o plano ao cliente e, se necessário, a um veículo e rastreador específicos." size="xl">
        <form className="space-y-5" onSubmit={submitContract}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <select className={fieldClass} value={contractForm.delivery_method} onChange={e => setContractForm(p => ({ ...p, delivery_method: e.target.value }))}><option value="email">Tipo de envio: Email</option><option value="whatsapp">Tipo de envio: WhatsApp</option><option value="todos">Tipo de envio: Todos</option></select>
