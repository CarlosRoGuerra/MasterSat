...
const toneConfig: Record<Tone, { card: string; icon: string; value: string }> = {
  ...
  brand:    { card: 'bg-brand-50 border-brand-200 dark:bg-brand-950/40 dark:border-brand-900/60', icon: 'bg-brand-100 text-brand-700 dark:bg-brand-900/60 dark:text-brand-300', value: 'text-brand-700 dark:text-brand-200' },
    ...
    <div className={clsx('rounded-2xl border p-5 shadow-card', cfg.card)}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="truncate text-2xs font-semibold uppercase tracking-[0.06em] text-slate-500 dark:text-slate-400">
            {label}
          </p>
          <p className={clsx('mt-2 text-stat font-semibold leading-none tabular-nums', cfg.value)}>
            {value}
          </p>
          <div className="mt-1.5 flex items-center gap-2">
            ...
            {delta && delta.value !== 0 && (
              <span className={clsx('text-2xs font-semibold tabular-nums', deltaColor)}>
        ...
        {icon && (
          <div className={clsx('flex h-11 w-11 shrink-0 items-center justify-center rounded-xl', cfg.icon)}>
            {icon}
          </div>
        )}
      </div>
